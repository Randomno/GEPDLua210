...
colors.joanna_default_color = make_rgb(0.2, 0.2, 0.32)
...
colors.guard_default_color = make_rgb(0.0, 1.0, 0.0) -- TODO: Replace with body color
...
function init_level_data()
	local data = parse_map_file("Maps/PD/CarringtonInstitute.map")
		
	for index, edge in ipairs(data.edges) do
		edge.color = colors.level_color
	end
	
	level_data["CarringtonInstitute"] = data
...
function update_mission()
	local current_mission = 0--GameData.get_current_mission()
...
function draw_guard(_guard_data_reader)
	...
	local color = (action_to_color[state.action] or colors.guard_default_color)--body_to_color[body_model])
...
function draw_joanna()
	...
	local color = colors.joanna_default_color
