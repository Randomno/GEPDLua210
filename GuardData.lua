...
guard_data =
{
	...
	{["offset"] = 0x024, ["size"] = 0x4, ["type"] = "float", 	["name"] = "collision_radius_1"},
	...
	{["offset"] = 0x070, ["size"] = 0x4, ["type"] = "float", 	["name"] = "distance_travelled_path"},
	{["offset"] = 0x074, ["size"] = 0x4, ["type"] = "float", 	["name"] = "segment_length_path"},	
	...
	{["offset"] = 0x094, ["size"] = 0x4, ["type"] = "float", 	["name"] = "distance_travelled"},
...
function read_guard_data_value(_slot, _name)
	local base_address = get_base_address(_slot)
	local metadata = guard_data_by_name[_name]

	if metadata.size == 1 then
		return mainmemory.read_u8(base_address + metadata.offset)
	elseif metadata.size == 2 then
		return mainmemory.read_u16_be(base_address + metadata.offset)
	elseif metadata.size == 4 then
		if metadata.type == "float" then
			return mainmemory.readfloat(base_address + metadata.offset, true)
		else
			return mainmemory.read_u32_be(base_address + metadata.offset)
		end
	else
